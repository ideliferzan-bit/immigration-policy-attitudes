...
asdoc tabstat imm_attitude econ_impact hh_income age education residence female ///
      mipex_change fb_change mipex_score fb_share_pct, ///
      stats(mean sd min max count) save("output/descriptives.doc") replace
...
twoway ///
  (line mipex_change year, sort lcolor(black) lwidth(medthick)) ///
  (line fb_change year, sort lcolor(gs8) lpattern(dash) lwidth(medthick)), ///
  yline(0, lcolor(gs15) lwidth(vthin)) ///
  yscale(range(0 2)) ///
  ylabel(0(0.5)2, nogrid noticks) ///
  xlabel(, noticks) ///
  ytitle("Change") ///
  xtitle("Year") ///
  legend(order(1 "MIPEX change" 2 "Foreign-born change") ///
         position(6) ring(1) cols(1) region(lstyle(none))) ///
...
esttab m0 m1 m2 m3 m4 using "output/immigration_models.html", ///
...
mixed econ_impact income_c lrscale_c female age_c education i.residence ///
      || cntry: || country_wave:
estimates store e1
...
esttab e0 e1 e2 e3 e4 using "output/economic_threat_models.html", ///
      replace se star(* 0.05 ** 0.01 *** 0.001) ///
      label compress ///
